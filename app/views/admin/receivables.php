    ...
    <div class="max-w-7xl mx-auto space-y-6">
        ...
        <div class="bg-white dark:bg-slate-800 p-5 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm">
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-4">
                <div>
                    <h2 class="text-2xl font-bold text-slate-800 dark:text-slate-100 tracking-tight">Monitoring Piutang</h2>
                    <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Pantau tagihan pelanggan (Kasbon) dan tandai lunas.</p>
                </div>
                <div class="flex gap-2">
                    <select id="filterMonth" onchange="generateDays()" class="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2 outline-none font-bold text-red-600 text-sm">
                        <?php 
                    ...
                    <select id="filterYear" onchange="generateDays()" class="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2 outline-none font-bold text-red-600 text-sm">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div class="bg-white dark:bg-slate-800 p-4 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm border-l-4 border-l-red-500">
                <p class="text-xs font-bold text-slate-500 uppercase">Total Piutang</p>
                <p class="text-2xl font-black text-red-600 mt-1" id="sumCount">0 <span class="text-sm font-medium text-red-400">Nota</span></p>
            </div>
            <div class="bg-white dark:bg-slate-800 p-4 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm border-l-4 border-l-red-500">
                <p class="text-xs font-bold text-slate-500 uppercase">Uang Tertahan</p>
                <p class="text-2xl font-black text-red-600 mt-1" id="sumTotal">Rp 0</p>
            </div>
        </div>
                ...
                <table class="min-w-full text-left text-sm whitespace-nowrap">
                    <thead class="bg-slate-50 dark:bg-slate-900/50 text-slate-500 dark:text-slate-400 font-bold uppercase tracking-wider border-b border-slate-200 dark:border-slate-700">
                        <tr>
                            ...
                            <th class="px-6 py-4 text-right">Tagihan Kasbon</th>
                            <th class="px-6 py-4 text-center">Aksi</th>
                        </tr>
                    </thead>
                    <tbody id="receivableGridBody" class="divide-y divide-slate-100 dark:divide-slate-700/50 text-slate-700 dark:text-slate-300">
...
<script>
    ...
    async function fetchReceivables() {
        const tbody = document.getElementById('receivableGridBody');
    ...
    function renderReceivables(receivables) {
        ...
        if (receivables.length === 0) {
            tbody.innerHTML = `<tr><td colspan="5" class="text-center py-12"><p class="font-bold text-slate-400">Bagus! Tidak ada piutang aktif pada tanggal ini.</p></td></tr>`;
            document.getElementById('sumCount').innerHTML = `0 <span class="text-sm font-medium text-red-400">Nota</span>`;
        ...
        receivables.forEach(inv => {
            ...
            inv.items.forEach(item => { itemsHtml += `<span class="text-xs bg-slate-100 dark:bg-slate-700/50 px-2 py-1 rounded w-max">${item.qty}x ${item.product_name}</span>`; });
            itemsHtml += `</div>`;

            tbody.innerHTML += `
                <tr class="hover:bg-red-50/50 dark:hover:bg-red-900/10 transition-colors">
                    <td class="px-6 py-4">
                        <div class="font-mono text-sm font-bold text-red-600 dark:text-red-400">${inv.invoice_number}</div>
                        <div class="text-xs text-slate-400 mt-1">${inv.created_at.split(' ')[1]} WIB</div>
                        ${inv.reference_no ? `<div class="text-[10px] mt-1 text-slate-400">Ref: ${inv.reference_no}</div>` : ''}
                    </td>
                    <td class="px-6 py-4 font-bold text-slate-800 dark:text-slate-200">${inv.customer_name || 'UMUM'}</td>
                    <td class="px-6 py-4">${itemsHtml}</td>
                    <td class="px-6 py-4 text-right font-black text-red-600 dark:text-red-400">${formatRp(inv.total_amount)}</td>
                    <td class="px-6 py-4 text-center flex flex-col gap-2">
                        <button onclick="processReceivableCompletion(${inv.id}, '${inv.invoice_number}')" class="bg-emerald-500 hover:bg-emerald-600 text-white font-bold py-2 px-4 rounded-lg text-xs transition duration-200 shadow-sm flex items-center justify-center gap-1 mx-auto w-full">
        ...
        document.getElementById('sumCount').innerHTML = `${receivables.length} <span class="text-sm font-medium text-red-400">Nota</span>`;
        document.getElementById('sumTotal').innerText = formatRp(totalAmount);
    }

    function processReceivableCompletion(id, invoiceStr) {
        Swal.fire({
            title: 'Lunasi Piutang?', html: `Tagihan <b>${invoiceStr}</b> akan ditandai Lunas.<br>Pilih metode pembayaran:`, icon: 'question',
